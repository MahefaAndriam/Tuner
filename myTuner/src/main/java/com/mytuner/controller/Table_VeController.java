...
import com.mytuner.model.Table_ve_Row;
...
public class Table_VeController implements Initializable, MapChangeListener<String, Double>, CommEventListener {
    Circle c ;
    ...
    private int[][] table_ve = {
            {30, 20, 20, 30, 20, 20, 20, 20, 20, 24, 24},
            { 22, 22, 23, 24, 24, 24, 24, 24, 24, 24, 24},
            { 26, 30, 27, 28, 28, 29, 29, 29, 29, 29, 29},
    ...
    private final List<int[][]> listOf_lastTableVe = new ArrayList<>();
    private final List<TableColumn<Table_ve_Row, Integer>> listTableCol = new LinkedList<>();

    @FXML private TableView<Table_ve_Row> table_veView;
    @FXML private BorderPane table_veViewParent;

    @FXML private TableColumn<Table_ve_Row, Integer> pression;
    @FXML private TableColumn<Table_ve_Row, Integer> col0;
    @FXML private TableColumn<Table_ve_Row, Integer> col1;
    @FXML private TableColumn<Table_ve_Row, Integer> col2;
    @FXML private TableColumn<Table_ve_Row, Integer> col3;
    @FXML private TableColumn<Table_ve_Row, Integer> col4;
    @FXML private TableColumn<Table_ve_Row, Integer> col5;
    @FXML private TableColumn<Table_ve_Row, Integer> col6;
    @FXML private TableColumn<Table_ve_Row, Integer> col7;
    @FXML private TableColumn<Table_ve_Row, Integer> col8;
    @FXML private TableColumn<Table_ve_Row, Integer> col9;
    @FXML private TableColumn<Table_ve_Row, Integer> col10;
    ...
    public Table_VeController(MyPort myPort) {
        this.myPort = myPort;
        int[][] table_ve_last0 = new int[11][11];
        listOf_lastTableVe.add(table_ve_last0);
        int[][] table_ve_last1 = new int[11][11];
        listOf_lastTableVe.add(table_ve_last1);
        int[][] table_ve_last2 = new int[11][11];
        listOf_lastTableVe.add(table_ve_last2);
        int[][] table_ve_last3 = new int[11][11];
        listOf_lastTableVe.add(table_ve_last3);
        int[][] table_ve_last4 = new int[11][11];
        listOf_lastTableVe.add(table_ve_last4);
    }

    private void updateTable(int addValue) {
        for(TablePosition pos : table_veView.getSelectionModel().getSelectedCells()){
            int rowIndex = pos.getRow();
            int colIndex = pos.getColumn();
            Table_ve_Row table_veView_row;
            ...
            try {
                int newValue = table_ve[rowIndex][colIndex - 1] + addValue;
    ...
    @FXML protected void update(){
        myPort.write_table(new byte[]{1}, table_ve);
    }
    private void upDate_tableVE() {
        for(int y = 0; y < table_ve.length; y++){
            System.arraycopy(table_ve[y], 0, listOf_lastTableVe.get(numOf_lastTable)[y], 0, table_ve[y].length);
        }
        numOf_lastTable++;
        ...
        int rowIndex = 0;
        for(Table_ve_Row table_Row : table_veView.getItems()){
            for(int i = 0; i < table_ve.length; i++) {
                table_ve[rowIndex][0] = table_Row.getCol0();
                table_ve[rowIndex][1] = table_Row.getCol1();
                table_ve[rowIndex][2] = table_Row.getCol2();
                table_ve[rowIndex][3] = table_Row.getCol3();
                table_ve[rowIndex][4] = table_Row.getCol4();
                table_ve[rowIndex][5] = table_Row.getCol5();
                table_ve[rowIndex][6] = table_Row.getCol6();
                table_ve[rowIndex][7] = table_Row.getCol7();
                table_ve[rowIndex][8] = table_Row.getCol8();
                table_ve[rowIndex][9] = table_Row.getCol9();
                table_ve[rowIndex][10] = table_Row.getCol10();
            }
            rowIndex++;
        }
    }
    private void update_tableVE_view() {
        int row = 0;
        for(Table_ve_Row table_row :table_veView.getItems()){
            table_row.set_table_ve_row(table_ve[row]);
            row++;
        }
        table_veView.refresh();
    }
    public ObservableList<Table_ve_Row> list = FXCollections.observableArrayList(
            new Table_ve_Row( pression_col[0], table_ve[0]),
            new Table_ve_Row( pression_col[1], table_ve[1]),
            new Table_ve_Row( pression_col[2], table_ve[2]),
            new Table_ve_Row( pression_col[3], table_ve[3]),
            new Table_ve_Row( pression_col[4], table_ve[4]),
            new Table_ve_Row( pression_col[5], table_ve[5]),
            new Table_ve_Row( pression_col[6], table_ve[6]),
            new Table_ve_Row( pression_col[7], table_ve[7]),
            new Table_ve_Row( pression_col[8], table_ve[8]),
            new Table_ve_Row( pression_col[9], table_ve[9]),
            new Table_ve_Row( pression_col[10], table_ve[10])
    );
    ...
    public void initialize(URL arg0, ResourceBundle arg1) {
        ...
        for(TableColumn<Table_ve_Row, Integer> col : listTableCol){
        ...
        table_veView.setOnKeyReleased(keyEvent -> {
            if(controlWasDonw && !keyEvent.isControlDown() && table_veView.getSelectionModel().getSelectedCells().size() > 0){
                AddValueVe addValueController = new AddValueVe();
        ...
        preChange.setOnMouseClicked(event -> {
            if(numOf_lastTable != 0){
                if(listOf_lastTableVe.get(numOf_lastTable - 1)[0][0] != 0){
                    table_ve = listOf_lastTableVe.get(numOf_lastTable - 1);
                    update_tableVE_view();
                }
                numOf_lastTable--;
            }else {
                if(listOf_lastTableVe.get(4)[0][0] != 0){
                    table_ve = listOf_lastTableVe.get(4);
                    numOf_lastTable = 4;
                    update_tableVE_view();
                }
            }
...
class AddValueVe {
    ...
    void showStage() {
        ...
        okToAddValue.setOnMouseClicked(event -> {
            newVal  = Integer.parseInt(addedValue.getText());
            stage.close();
        });
